  ...
  "Email Connector" must {
    "return an EmailSent" when {
      "email sent successfully with status 202 (Accepted)" in {
        ...
        connector.sendEmail(requestId, testPspId, journeyType, testEmailAddress, testTemplate, Map.empty).map { result => result mustBe EmailSent
        }
      }
    }
    ...
    "return an EmailNotSent" when {
      "email service is down" in {
        ...
        connector.sendEmail(requestId, testPspId, journeyType, testEmailAddress, testTemplate, Map.empty).map { result => result mustBe EmailNotSent
        }
      }
