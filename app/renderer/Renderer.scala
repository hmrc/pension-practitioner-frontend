...
class Renderer @Inject()(appConfig: FrontendAppConfig, renderer: NunjucksRenderer) {
  ...
  def render[A](template: String, ctx: A)
               (implicit request: RequestHeader, writes: OWrites[A]): Future[Html] =
    renderTemplate(template, Json.toJsObject(ctx))

  def render(template: String, ctx: JsObject)
            (implicit request: RequestHeader): Future[Html] =
    renderTemplate(template, ctx)

  private def renderTemplate(template: String, ctx: JsObject)
                            (implicit request: RequestHeader): Future[Html] =
    renderer.render(
      template = template,
      ctx = ctx ++ Json.obj("config" -> config.+("nonce" -> JsString(CSPNonce.get.getOrElse(""))))
    )
