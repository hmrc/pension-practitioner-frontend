...
object DataRetrievals {
  ...
  def retrievePspNameAndEmail(block: (String, String) => Future[Result])(implicit request: DataRequest[AnyContent]): Future[Result] = {
    ...
    (ua.get(WhatTypeBusinessPage), ua.get(IndividualDetailsPage), ua.get(BusinessNamePage), ua.get(IndividualEmailPage), ua.get(CompanyEmailPage)) match {
      case (Some(Yourselfasindividual), Some(i), _, Some(e), _) => block(i.fullName, e)
      case (Some(Companyorpartnership), _, Some(b), _, Some(e)) => block(b, e)
